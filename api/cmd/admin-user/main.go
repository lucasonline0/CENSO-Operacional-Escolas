...
func main() {
	...
	case "create":
		...
		canonical, err := appModels.DREs.GetByNome(ctx, *dre)
		if err != nil {
			log.Fatalf("Erro ao localizar DRE: %v", err)
		}
		user, err := appModels.AdminUsers.ProvisionForDREID(ctx, *username, *email, pwd, "dre", canonical.ID)
		if err != nil {
	...
	case "update-password":
		...
		user, err := appModels.AdminUsers.GetByUsername(ctx, *username)
		if err == nil {
			err = appModels.AdminUsers.ResetTemporaryPasswordByID(ctx, user.ID, pwd)
		}
		if err != nil {
			log.Fatalf("Erro ao atualizar senha: %v", err)
		}

		fmt.Printf("Credencial temporária do usuário '%s' atualizada; troca obrigatória no próximo acesso.\n", *username)
	...
	case "list":
		...
		fmt.Printf("%-5s | %-20s | %-32s | %-10s | %-30s | %-16s | %-20s\n", "ID", "USERNAME", "EMAIL", "ROLE", "DRE", "CREDENCIAL", "CRIADO EM")
		fmt.Println(strings.Repeat("-", 150))
		for _, u := range users {
			credentialStatus := "Acesso ativo"
			if !u.Active {
				credentialStatus = "Usuário inativo"
			} else if u.MustChangePassword {
				credentialStatus = "Primeiro acesso"
			}
			fmt.Printf("%-5d | %-20s | %-32s | %-10s | %-30s | %-16s | %-20s\n",
				u.ID, u.Username, u.Email, u.Role, u.DRE, credentialStatus, u.CreatedAt.Format("02/01/2006 15:04"))
		}
