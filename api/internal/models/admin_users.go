...
// ValidateDRE verifica se a DRE existe na tabela de escolas.
func (m *AdminUserModel) ValidateDRE(ctx context.Context, dre string) (bool, error) {
	query := `SELECT EXISTS(SELECT 1 FROM schools WHERE UPPER(TRIM(dre)) = UPPER(TRIM($1)))`
	var exists bool
	err := m.DB.QueryRowContext(ctx, query, strings.TrimSpace(dre)).Scan(&exists)
	return exists, err
}
