...
| Alta | Caracterização da Rede | Infraestrutura Educacional | Menu/anchor existem, mas o bloco está vazio; há `vw_censo_ambientes`, sem endpoint sintético e sem lista oficial de ambientes essenciais. | Produto | Definir ambientes essenciais; depois criar endpoint e bloco sintético. |
...
| Infraestrutura Educacional | Presença de ambientes | Menu e `sec-perfil-infra` OK | Anchor vazio | Não há endpoint de Caracterização para ambientes | `vw_censo_ambientes.ambiente` existe | Backend + Frontend | Reaproveitar `vw_censo_ambientes` em endpoint sintético `caracterizacao/infraestrutura-educacional` | Backend |
| Infraestrutura Educacional | Cobertura de ambientes essenciais | Menu e anchor OK | Anchor vazio | Não há payload | `vw_censo_ambientes` não marca essencialidade | Produto | Definir lista oficial de ambientes essenciais antes de endpoint/UI | Produto/Dados |
| Infraestrutura Educacional | Média de ambientes essenciais por porte | Menu e anchor OK | Anchor vazio | Não há payload | Depende de essencialidade + porte | Produto | Após definição de essenciais, expor média por `porte_escola_nome` | Produto/Dados |
...
- Infraestrutura Educacional ainda não existe como conteúdo. Pode reaproveitar `vw_censo_ambientes`, mas depende de decisão de produto para "ambientes essenciais".
