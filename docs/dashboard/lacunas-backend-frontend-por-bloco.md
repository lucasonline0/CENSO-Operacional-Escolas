...
| Entregue | Serviços Terceirizados | Manipulador de Alimentos | MER-RH-01 migrou RH de merendeiras para Serviços Terceirizados, removeu `sec-merenda-rh` de Merenda e manteve `/merenda/recursos-humanos` como legado. | Backend + Frontend + Docs | Manter; avaliação do serviço das merendeiras fica pendente por fonte/escala. |
...
Blocos oficiais realinhados aos gráficos mínimos do Data Studio (ver `matriz-abas-e-graficos.md` §5.5): **Oferta e Adequação da Merenda · Estrutura Física · Equipamentos da Merenda · Condições Sanitárias e Segurança**. O antigo bloco **Recursos Humanos** deixou de ser finalístico de Merenda e foi migrado para **Serviços Terceirizados** ("Manipulador de Alimentos") em MER-RH-01.
...
| Equipamentos da Merenda | Estado de conservação consolidado | Menu e anchor OK | Barras empilhadas horizontais 100% (Bom/Regular/Ruim-Inoperante) + tabela detalhada | `/merenda/equipamentos` entrega `estado_consolidado` (+ `dist_estados` mantida) | `estado_freezers`, `estado_geladeiras`, `estado_fogoes`, `estado_fornos`, `estado_bebedouros` | Sem lacuna (MER-01B; refinamento visual MER-EQP-REFINE-01) | Manter | — |
...
Nota — **Recursos Humanos / merendeiras (fora da matriz finalística de Merenda).** MER-RH-01 removeu o bloco RH de `AbaMerenda.tsx` (`sec-merenda-rh`) e adicionou o bloco **"Manipulador de Alimentos"** em `AbaServicosTerceirizados.tsx` (`sec-servicos-manipuladores`), ao lado de Serviços Gerais e Portaria. A fonte `vw_censo_rh_merendeiras` permanece ativa; `/v1/admin/analytics/servicos-terceirizados/manipuladores-alimentos` é o endpoint novo e `/v1/admin/analytics/merenda/recursos-humanos` permanece legado.

Diagnóstico B2 — Merenda (atualizado): os quatro blocos finalísticos da matriz mínima do Data Studio estão **entregues** — **Oferta e Adequação** e **Estrutura Física** (MER-01A), **Equipamentos da Merenda** (MER-01B) e **Condições Sanitárias e Segurança** (MER-01C, via `/merenda/condicoes-sanitarias` sobre `vw_censo_equipamentos_merenda`). A pendência não finalística de RH/Merendeiras foi resolvida em MER-RH-01 com a migração para Serviços Terceirizados; permanece apenas a avaliação do serviço das merendeiras como pendência futura/produto.
...
| Manipulador de Alimentos | RH merendeiras/manipuladores | `sec-servicos-manipuladores` e submenu OK | KPIs + Donuts + HBar | `GET /v1/admin/analytics/servicos-terceirizados/manipuladores-alimentos` | `vw_censo_rh_merendeiras` | Entregue (MER-RH-01) | Manter; `/merenda/recursos-humanos` legado | — |
...
- Manter o bloco "Manipulador de Alimentos" entregue em MER-RH-01 e tratar a avaliação do serviço das merendeiras apenas quando houver fonte/escala consolidada.
