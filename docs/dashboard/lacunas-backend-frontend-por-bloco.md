...
| Média/Alta | Merenda Escolar | Condições Sanitárias e Segurança | **Bloco ausente** (sem endpoint, sem frontend); campos já existem em `vw_censo_equipamentos_merenda` (`despensa_exclusiva`, `deposito_conserva`, `possui_balanca`, `bancadas_inox`, `sistema_exaustao`, `estoque_epi_extintor`, `manutencao_extintores`). | Backend + Frontend | Criar endpoint dedicado (ex.: `/v1/admin/analytics/merenda/condicoes-sanitarias`) ou expandir `/merenda/oferta`; renderizar bloco. |
| Média/Alta | Merenda Escolar | Equipamentos da Merenda | Gráficos mínimos do Data Studio ainda não totalmente refletidos: presença por tipo, escolas com 1/2/3+ equipamentos, estado consolidado e criticidade. | Backend + Frontend | Expor presença/quantidade/criticidade por tipo após validar faixas e definição de criticidade com produto. |
| Média | Merenda Escolar | Estrutura Física | Condições da cozinha e refeitório (KPI) existem; `tamanho_cozinha` e `refeitorio_adequado` estão na view, mas não no payload/frontend; `possui_refeitorio` só como KPI %. | Backend + Frontend | Expor `tamanho_cozinha`, `refeitorio_adequado` e distribuição Sim/Não de `possui_refeitorio`; renderizar. |
| Média | Merenda Escolar | Oferta e Adequação da Merenda | "Atende necessidades" só como KPI % (`pct_atende_necessidades`); falta a distribuição completa Sim/Parcialmente/Não. | Backend + Frontend | Expor distribuição categórica de `atende_necessidades` e renderizar donut/barra. |
...
| Oferta e Adequação da Merenda | Merenda atende necessidades — distribuição Sim/Parcialmente/Não | Menu e anchor OK | KPI % no resumo executivo | `/merenda/oferta` entrega só `pct_atende_necessidades` (% "Sim") | `vw_censo_rh_merendeiras.atende_necessidades` | Backend + Frontend | Expor distribuição categórica completa e renderizar donut/barra | Backend |
...
| Estrutura Física | Possui refeitório — distribuição Sim/Não | Menu e anchor OK | KPI % renderizado | `/merenda/oferta` entrega só `pct_possui_refeitorio` | `vw_censo_equipamentos_merenda.possui_refeitorio` | Backend + Frontend | Expor distribuição Sim/Não e renderizar (manter KPI) | Backend |
...
| Estrutura Física | Refeitório atende adequadamente | Menu e anchor OK | Não renderizado | `/merenda/oferta` não expõe | `vw_censo_equipamentos_merenda.refeitorio_adequado` | Backend + Frontend | Expor distribuição de `refeitorio_adequado` e renderizar no bloco | Backend |
...
| Equipamentos da Merenda | Presença de equipamentos por tipo (% de escolas) | Menu e anchor OK | Não renderizado | Não expõe % de escolas com `qtd_* > 0` | `qtd_*` por tipo | Backend + Frontend | Expor presença por tipo e renderizar | Backend |
| Equipamentos da Merenda | Escolas com 1, 2 ou 3+ equipamentos | Menu e anchor OK | Não renderizado | Não expõe distribuição por faixa | `qtd_*` por tipo | Backend + Frontend (+ Produto) | Definir faixas oficiais; expor distribuição e renderizar | Produto/Dados |
| Equipamentos da Merenda | Estado de conservação consolidado | Menu e anchor OK | Tabela por equipamento×estado renderizada | `/merenda/equipamentos` entrega `dist_estados` (por equipamento) | `estado_freezers`, `estado_geladeiras`, `estado_fogoes`, `estado_fornos`, `estado_bebedouros` | Frontend (parcial) | Adicionar visão consolidada além da tabela atual | Frontend |
| Equipamentos da Merenda | Criticidade por equipamento | Menu e anchor OK | Não renderizado | Não expõe índice de criticidade | `estado_*` por tipo | Produto, depois Backend + Frontend | Definir "criticidade" (ex.: % ruim/inservível) e então expor | Produto/Dados |
| Condições Sanitárias e Segurança | Despensa exclusiva para gêneros alimentícios | **Sem menu/anchor** | Bloco ausente | Não há endpoint/payload | `vw_censo_equipamentos_merenda.despensa_exclusiva` | Backend + Frontend | Criar endpoint (ex.: `/merenda/condicoes-sanitarias`) ou expandir `/merenda/oferta`; criar bloco | Backend |
| Condições Sanitárias e Segurança | Depósito conserva adequadamente os alimentos | **Sem menu/anchor** | Bloco ausente | Não há endpoint/payload | `vw_censo_equipamentos_merenda.deposito_conserva` | Backend + Frontend | Idem | Backend |
| Condições Sanitárias e Segurança | Presença de itens básicos (balança, bancadas inox, exaustão) | **Sem menu/anchor** | Bloco ausente | Não há endpoint/payload | `possui_balanca`, `bancadas_inox`, `sistema_exaustao` | Backend + Frontend | Idem | Backend |
| Condições Sanitárias e Segurança | Estoque de EPIs e extintor de incêndio | **Sem menu/anchor** | Bloco ausente | Não há endpoint/payload | `estoque_epi_extintor` | Backend + Frontend | Idem | Backend |
| Condições Sanitárias e Segurança | Recarga/manutenção dos extintores | **Sem menu/anchor** | Bloco ausente | Não há endpoint/payload | `manutencao_extintores` | Backend + Frontend | Idem | Backend |
...
Diagnóstico B2 — Merenda (atualizado): a aba está estruturalmente entregue em Oferta, Estrutura Física (parcial) e Equipamentos (parcial), mas **não reflete totalmente os gráficos mínimos do Data Studio**: faltam distribuições completas em Oferta/Estrutura, presença/quantidade/criticidade em Equipamentos e o **bloco inteiro de Condições Sanitárias e Segurança** (campos já existem em `vw_censo_equipamentos_merenda`, sem endpoint nem frontend).
...
- Expor, na Merenda — Estrutura Física, a distribuição de `tamanho_cozinha`, `refeitorio_adequado` e a distribuição Sim/Não de `possui_refeitorio` (campos já existem em `vw_censo_equipamentos_merenda`).
- Expor, na Merenda — Oferta, a distribuição completa Sim/Parcialmente/Não de `atende_necessidades` (hoje só `pct_atende_necessidades`).
...
- Criar o bloco **Merenda — Condições Sanitárias e Segurança** (bloco ausente): endpoint dedicado (ex.: `/v1/admin/analytics/merenda/condicoes-sanitarias`) ou expansão controlada de `/merenda/oferta`, expondo `despensa_exclusiva`, `deposito_conserva`, `possui_balanca`, `bancadas_inox`, `sistema_exaustao`, `estoque_epi_extintor`, `manutencao_extintores` — todos já presentes em `vw_censo_equipamentos_merenda`.
...
- Renderizar, em Merenda, as distribuições faltantes: tamanho da cozinha, refeitório adequado, Sim/Não de possui refeitório, distribuição Sim/Parcialmente/Não de "atende necessidades", presença/quantidade/estado consolidado de equipamentos e o novo bloco Condições Sanitárias e Segurança (quando o backend expuser payload).
