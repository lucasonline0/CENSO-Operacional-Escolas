...
> **Nota sobre Recursos Humanos / merendeiras.** O painel original do Data Studio mantinha em Merenda uma subaba **"Recursos Humanos"** (total de merendeiras por vínculo, adequação do quantitativo, avaliação do serviço, média por escola, empresas e supervisão). Por decisão de produto (§2.8), esse bloco **não é bloco finalístico da aba Merenda** e foi migrado para o menu **Serviços Terceirizados**, como bloco **"Manipulador de Alimentos"**, ao lado de Serviços Gerais e Portaria. Por isso ele **não aparece** na lista de blocos oficiais de Merenda. O endpoint `/v1/admin/analytics/merenda/recursos-humanos` permanece ativo como legado; a UI não o consome mais em Merenda.
>
...
| Equipamentos da Merenda | Estado de conservação consolidado | PG `/merenda/equipamentos` (`estado_consolidado`) | presente (MER-01B) | — | — | Agrupado em Bom/Regular/Ruim-Inoperante por equipamento; refinado de tabela compacta para **gráfico de barras empilhadas horizontais** (100% por equipamento) acima da tabela detalhada (`dist_estados` mantida) — MER-EQP-REFINE-01. |
...
**Recursos Humanos (fora da matriz finalística da aba Merenda).** Os indicadores de RH/merendeiras — total de merendeiras estatutárias/terceirizadas/temporárias, adequação do quantitativo, média por escola, empresas e supervisão — **não compõem mais a matriz mínima de Merenda Escolar**. Eles foram migrados para **Serviços Terceirizados**, como bloco **"Manipulador de Alimentos"** (§2.8 e §5.6). O endpoint `/merenda/recursos-humanos` permanece legado; o bloco `sec-merenda-rh` foi removido da UI. A avaliação do serviço permanece fora do escopo por falta de fonte/escala consolidada.
...
> **Bloco entregue — Manipulador de Alimentos.** MER-RH-01 adicionou `GET /v1/admin/analytics/servicos-terceirizados/manipuladores-alimentos` e o bloco `sec-servicos-manipuladores`, usando `vw_censo_rh_merendeiras`. O payload cobre totais por vínculo, total geral, média por escola, % com supervisor, distribuição por vínculo, atendimento à necessidade e top empresas. `/merenda/recursos-humanos` permanece legado.
...
| Manipulador de Alimentos | Total por vínculo (KPIs) | PG `/servicos-terceirizados/manipuladores-alimentos` | presente (MER-RH-01) | — | — | Usa `vw_censo_rh_merendeiras`. |
| Manipulador de Alimentos | Distribuição por vínculo (donut) | PG `/servicos-terceirizados/manipuladores-alimentos` | presente (MER-RH-01) | — | — | Composição percentual dos quantitativos somados por vínculo. |
| Manipulador de Alimentos | Quantidade atual atende à necessidade? (donut) | PG `/servicos-terceirizados/manipuladores-alimentos` | presente (MER-RH-01) | — | — | Distribuição de escolas por `atende_necessidades`. |
| Manipulador de Alimentos | Top empresas terceirizadas (barra) | PG `/servicos-terceirizados/manipuladores-alimentos` | presente (MER-RH-01) | — | — | Campo textual; variações de grafia podem aparecer separadamente. |
| Manipulador de Alimentos | Avaliação do serviço das merendeiras | — | pendente | Fonte/payload a definir | Sim | Fora de escopo de MER-RH-01. |
...
- **Merenda — Recursos Humanos:** **MER-RH-01 entregue**. Não é mais lacuna de Merenda; o bloco foi movido para Serviços Terceirizados, e `/merenda/recursos-humanos` permanece como endpoint legado.
