...
export function AbaCaracterizacao({
  token,
  onUnauth,
  filters,
  ...
  useEffect(() => {
    ...
    const pPerfil = apiFetch<CaracterizacaoPerfilPg>(
      `/v1/admin/analytics/caracterizacao/perfil${qs}`,
      token
    )
      .then((d) => {
        if (!cancelled) setPerfilPg(d);
      })
      .catch(handleErr(setPerfilErr));

    const pDre = apiFetch<CaracterizacaoDREPg>(
      `/v1/admin/analytics/caracterizacao/dre${qs}`,
      token
    )
      .then((d) => {
        if (!cancelled) setDrePg(d);
      })
      .catch(handleErr(setDreErr));

    const pOferta = apiFetch<CaracterizacaoOfertaFuncionamento>(
      `/v1/admin/analytics/caracterizacao/oferta-funcionamento${qs}`,
      token
    )
      .then((d) => {
        if (!cancelled) setOfertaPg(d);
      })
      .catch(handleErr(setOfertaErr));

    const pInfra = apiFetch<CaracterizacaoInfraEducacionalPg>(
      `/v1/admin/analytics/caracterizacao/infraestrutura-educacional${qs}`,
      token
    )
      .then((d) => {
        if (!cancelled) setInfraPg(d);
      })
      .catch(handleErr(setInfraErr));

    const pSheet = apiFetch<SheetMetrics>(
  ...
  const sourceLabel =
    usePerfilPg && useDrePg
      ? buildPostgresSourceLabel(filters)
      : !usePerfilPg && !useDrePg
  ...
  return (
    <div className="space-y-6">
      ...
      {!presentationMode && (perfilErr || dreErr) && (
      ...
      {!presentationMode && sheetErr && !perfilErr && !dreErr && (
      ...
      {isVisible("sec-perfil-dimensao") && (
        <div className={presentationMode ? "space-y-4" : "space-y-6"}>
          <div id="sec-perfil-dimensao" className="animate-fade-in-up">
            ...
            <div className={`grid grid-cols-2 lg:grid-cols-4 ${presentationMode ? "gap-4" : "gap-4"}`}>
          ...
          <div className={presentationMode ? "grid grid-cols-2 gap-5" : "space-y-5"}>
            <div className={`ca-hbar-pair bg-white rounded-2xl border border-slate-200 shadow-sm animate-fade-in-up [animation-delay:300ms] ${presentationMode ? "p-6" : "p-6"}`}>
              <h3 className={`font-semibold text-slate-800 text-sm flex items-center gap-2 ${presentationMode ? "mb-5" : "mb-5"}`}>
            ...
            <div className={`ca-hbar-pair bg-white rounded-2xl border border-slate-200 shadow-sm animate-fade-in-up [animation-delay:450ms] ${presentationMode ? "p-6" : "p-6"}`}>
              <h3 className={`font-semibold text-slate-800 text-sm flex items-center gap-2 ${presentationMode ? "mb-5" : "mb-5"}`}>
                <MapPinned size={16} style={{ color: C.primary }} />
      ...
      {isVisible("sec-perfil-oferta") && (
        <div id="sec-perfil-oferta" className="animate-fade-in-up [animation-delay:600ms] space-y-6">
      ...
      {isVisible("sec-perfil-infra") && (
        <div id="sec-perfil-infra" className="animate-fade-in-up [animation-delay:750ms] space-y-6">
          <div className="flex items-center gap-3 mb-4">
